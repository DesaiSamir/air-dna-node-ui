...
var TradeSetup = function(record){
    this.app_order_id = record.app_order_id;
    this.management_type_id = record.management_type_id;
...
TradeSetup.getManagementTypes = async function() {
    const query = `SELECT * FROM management_type_ref`;
...
TradeSetup.getTradeSetups = async function() {
    const query = `SELECT * FROM trade_setup_orders`;
    const result = await db.getData(query);
